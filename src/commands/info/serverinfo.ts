...
class ServerInfo extends Command {
	...
	async run(message: Message) {
		...
		const stats = {
			...
			voiceChannels:guild?.channels.cache.filter((c) => c.type === 'GUILD_VOICE').size || 'No Data',
			stageChannels:guild?.channels.cache.filter((c) => c.type === 'GUILD_STAGE_VOICE').size || 'No Data',
			publicThreads:guild?.channels.cache.filter((c) => c.type === 'GUILD_PUBLIC_THREAD').size || 'No Data',
		};
		...
		const serverInfoEmbed = new MessageEmbed()
			...
			.addFields(
				{
					name:'General',
					value:`:white_small_square: **Server Id :**${guild?.id}\n:white_small_square: **Owner :** ${owner?.user.tag}(${owner?.id})\n:white_small_square: **Created :** ${new Date(guild?.createdTimestamp || '').toDateString()}\n:white_small_square: **Boosts :** ${premiumTier}\n :white_small_square: **Partnered :** ${guild?.partnered ? '<:CryptonDPartner:869132408463695902> Yes' : 'No'}\n:white_small_square: **Verified :** ${guild?.verified ? '<:CryptonDVerified:871322249792221224> Yes' : 'No'}`,
			
				},
				{
					name:'Stats',
					inline:true,
					value:`:white_small_square: **Role Count :** ${stats.roleCount}\n:white_small_square: **Emoji Count :** ${stats.emojiCount}\n:white_small_square: **Stickers :** ${stats.stickerCount}\n:white_small_square: **Total Members :** ${stats.bots + stats.humans}\n:white_small_square: **Humans :** ${stats.humans}\n:white_small_square: **Bots :** ${stats.bots}\n:white_small_square: **Text Channels :** ${stats.textChannels}\n:white_small_square: **Voice Channels :** ${stats.voiceChannels}\n:white_small_square: **Stages :** ${stats.stageChannels}\n:white_small_square: **Public Threads :** ${stats.publicThreads}`,
				},
				{
					name:'Crypton',
					inline:true,
					value:':white_small_square: **Premium :** No\n:white_small_square: **Enabled Commands :** 0\n:white_small_square: **Dashboard :** [Click Here](https://www.youtube.com/watch?v=dQw4w9WgXcQ)',
				},
				{
					name:'Features',
